...
export default function DomeGallery({
  ...
  // Auto-rotate slowly — pauses during drag, inertia, or enlarged view
  const autoRotateRAF = useRef<number | null>(null);
  useEffect(() => {
    const AUTO_SPEED = 0.015; // degrees per frame (~0.9°/sec at 60fps)
    ...
    const tick = () => {
      ...
      const isFocused = focusedElRef.current !== null;

      if (!isDragging && !hasInertia && !isEnlarged && !isFocused) {
        const speed = AUTO_SPEED * (dt / 16.667); // normalize to 60fps
